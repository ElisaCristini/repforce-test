...
export function CatalogPage() {
  ...
  const filters = {
    ...
    category: (search as any).category ?? "",
    brand: (search as any).brand ?? "",
    minPrice: (search as any).minPrice ?? "",
    maxPrice: (search as any).maxPrice ?? "",
    inStock: (search as any).inStock === "true",
    page: Number((search as any).page ?? 1),
  };

  function updateFilters(updates: Record<string, string | number | boolean | undefined>) {
    navigate({
      search: (prev: any) => ({
        ...prev,
        ...updates,
    ...
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="bg-muted/40 rounded-lg p-4 mb-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="space-y-1">
          ...
          <Select
            value={filters.brand}
            onValueChange={(v) => updateFilters({ brand: v === "all" ? undefined : v })}
          >
            <SelectTrigger>
              <SelectValue placeholder="Todas" />
            </SelectTrigger>
        ...
        <div className="flex items-center gap-6 sm:col-span-2 lg:col-span-4">
          <div className="flex items-center gap-2">
            <Checkbox
              ...
              onCheckedChange={(v) => updateFilters({ inStock: v ? "true" : undefined })}
            />
                ...
                <CardContent className="p-4 space-y-2">
                  ...
                  <div className="flex items-center justify-between pt-2">
                    ...
                    <Button
                      size="sm"
                      onClick={() =>
                        navigate({ to: "/produtos/$id", params: { id: product.id } } as any)
                      }
                    >
                      Ver detalhes
                    </Button>
